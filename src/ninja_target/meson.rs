...
pub fn meson_setup(
    ...
    let dirs_args = vec![path_to_string(build_path), path_to_string(src_path)];
    args.extend(meson_args);
    args.extend(dirs_args.iter().map(|arg| arg.as_str()));
    execute_cmd!("meson", args, env_vars)
}

pub fn meson_compile(build_path: &Path) -> Result<(), String> {
    execute_cmd!(
        "meson",
        vec!["compile", "-C", &path_to_string(build_path)],
        None
    )
}
...
pub fn get_targets(
    ...
    env_vars: Option<Vec<(&str, &str)>>,
    ctx: &Context,
) -> Result<Vec<MesonNinjaTarget>, String> {
    if !ctx.skip_gen_ninja {
        meson_setup(src_path, build_path, meson_args, env_vars)?;
        if !ctx.skip_build {
            meson_compile(build_path)?;
        }
    }
