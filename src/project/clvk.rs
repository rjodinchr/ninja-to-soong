...
impl<'a> CLVK<'a> {
    pub fn new(android_root: &'a str, temp_dir: &'a str, ndk_root: &'a str) -> Self {
        CLVK {
            src_root: clvk_dir(android_root),
...
impl<'a> crate::project::Project<'a> for CLVK<'a> {
    ...
    fn get_build_directory(&self) -> Result<String, String> {
        let spirv_headers_dir =
            "-DSPIRV_HEADERS_SOURCE_DIR=".to_string() + &spirv_headers_dir(self.android_root);
        let spirv_tools_dir =
            "-DSPIRV_TOOLS_SOURCE_DIR=".to_string() + &spirv_tools_dir(self.android_root);
        let clspv_dir = "-DCLSPV_SOURCE_DIR=".to_string() + &clspv_dir(self.android_root);
        let llvm_project_dir = llvm_project_dir(self.android_root);
